...
public interface IDashboardService
{
    ...
    /// Se <paramref name="clienteId"/> è specificato, i valori sono ristretti al solo fatturato di quel cliente.
    /// Se <paramref name="fornitoreIds"/> è specificato (uno o più), i valori sono ristretti al fatturato
    /// di quei fornitori (es. Nobilia, NobiSmart...).</summary>
    Task<DashboardKpiDto> GetKpiPrincipaliAsync(
    ...
    /// la serie riporta solo il fatturato di quel cliente. Se <paramref name="fornitoreIds"/> è
    /// specificato, riporta solo il fatturato di quei fornitori.</summary>
    Task<IReadOnlyList<PuntoGraficoMensileDto>> GetFatturatoMensileAsync(
        int anno, int? agenteId = null, int? clienteId = null,
        IReadOnlyList<int>? fornitoreIds = null, CancellationToken ct = default);

    /// <summary>Fatturato e provvigione per cliente (portafoglio di un agente, o singolo cliente) nei
    /// mesi/anno indicati, opzionalmente ristretto a uno o più fornitori.</summary>
